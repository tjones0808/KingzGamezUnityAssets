...
namespace EasyBuildSystem.Runtimes.Internal.Part
{
    ...
    public class PartBehaviour : MonoBehaviour
    {
                ...
                for (int i = 0; i < CustomDetections.Length; i++)
                {
                    if (CustomDetections[i] != null)
                    {
                        ...
                        for (int x = 0; x < Colliders.Length; x++)
                        {
                            ...
                            if (UseTerrain)
                            {
                                if (CustomDetections[i].RequiredSupports.Contains(SurfaceType.Surface))
                                {
                                    if (CustomDetections[i].RequiredSupports.ToList().Find(entry => entry == SurfaceType.Surface) == SurfaceType.Surface)
                                    {
                            ...
                            {
                                if (CustomDetections[i].RequiredSupports.ToList().Contains(SurfaceType.Surface))
                                {
        ...
        {
            if (BuildManager.Instance.BuildingSupport == SupportType.Static)
