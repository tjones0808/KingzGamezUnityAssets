...
namespace EasyBuildSystem.Runtimes.Internal.Builder
{
    ...
    public class BuilderBehaviour : MonoBehaviour
    {
        ...
        {
            get { return transform.parent != null ? transform.parent : transform; }
        }
        ...
        private void OnEnable()
        {
            if (!Application.isPlaying)
            {
                ...
                InputsCollection DefaultCollection = Resources.Load<InputsCollection>(Constants.DEFAULT_INPUT_COLLECTION);

                if (InputsCollection == null && DefaultCollection != null)
                    InputsCollection = DefaultCollection;
            }
        }
        ...
        public void UpdateSnapsMovement(SocketBehaviour[] sockets)
        {
                    ...
                    if (Socket != null)
                    {
                        if (Socket.gameObject.activeSelf && !Socket.IsDisabled)
                        {
                            if (Socket.AllowPart(SelectedPrefab) && !Part.AvoidAnchoredOnSocket)
                            {
                                if (RayDetection == DetectionType.Overlap)
                                {
                                    if ((Socket.transform.position - GetTransform.position).sqrMagnitude < Mathf.Pow(ActionDistance, 2))
                                    {
            ...
            if (CurrentSocket != null)
            {
                ...
                if (Offset != null)
                {
                    ...
                    CurrentPreview.transform.localScale = Offset.UseCustomScale ? 
                        Offset.Scale : (CurrentSocket.AttachedPart != null ? CurrentSocket.AttachedPart.transform.localScale : CurrentSocket.transform.localScale);
