...
public class UICircularMenu : MonoBehaviour
{
...
#if WINTERBYTE
        BuildingWheel.Player.SelectBuildable.TryStop();
#endif
    }
    ...
    public void RepairPrefab()
    {
        #if WINTERBYTE
        BasePart PartAtRepair = BaseBuilder.Instance.GetTargetedPart();

        if (PartAtRepair != null)
            if (PartAtRepair.GetComponent<CustomGenericPart>() != null)
                    PartAtRepair.GetComponent<CustomGenericPart>().RepairPartByAppearanceIndex(PartAtRepair.AppearanceIndex);
        #endif
    }
    ...
    {
#if WINTERBYTE
        BasePart PartAtUpgrade = BaseBuilder.Instance.GetTargetedPart();

        if (PartAtUpgrade != null)
            if (PartAtUpgrade.GetComponent<CustomGenericPart>() != null)
                PartAtUpgrade.GetComponent<CustomGenericPart>().UpgradePartByAppearanceIndex(appearanceIndex);
#else
